...
// SUBSEQUENCE - A subsequence is any subset of the array that maintains
// relative order but can skip elements. SUBARRAY - A subarray is a contiguous
// part of the array. It must maintain the order of elements and cannot skip
// elements.
...
class Solution {
    ...
    public boolean subsequence(
        ...
        subs.add(arr[idx]);
        sum += arr[idx];
        if (subsequence(arr, k, subs, idx + 1, sum))
            return true;
        // subs.removeLast();
        // OR
        subs.remove(subs.size() - 1);
